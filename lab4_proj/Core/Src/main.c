...
static bool letterEntered=false;
static bool letterEntered1=false;


 static int count=0;
static char readVal_interrupt2;
static char readVal_interrupt1;
...
//		int count1=0;
	 void fxnAcceptsArrayOfCharsAndReturnNothing_1(volatile char arr[] ){
...
//				count1++;
 ...
 void fxnAcceptsArrayOfCharsAndReturnNothing(volatile char arr[] ){
...
void turn_On_Off_Or_toggleLEDsWithTheFirstColorLetterAnd0To2Digits(char c, char i){
	 volatile char orangeLED[]={'O','r','a','n','g','e',' ','L','E','D','\n','\n'};
 
 volatile char redLED[]={'R','e','d',' ','L','E','D','\n','\n'};
 volatile char blueLED[]={'B','l','u','e',' ','L','E','D','\n','\n'};

 volatile char greenLED[]={'G','r','e','e','n',' ','L','E','D','\n','\n'};





 volatile char  errorMessageWrongButton[]={'\n','I','n','c','o','r','r','e','c','t',' ',
 'b','u','t','t','o','n',' ','i','s',' ','c','l','i','c','k','e','d','\n',
	 'c','l','i','c','k',':','\n', '"','r','"',' ','f','o','r',' ','R','e','d','\n',
	 '"','b','"',' ','f','o','r',' ','B','l','u','e','\n',
  '"','o','"',' ','f','o','r',' ','O','r','a','n','g','e','\n',
...
//else if (readVal_interrupt2 =='0' | readVal_interrupt2 == '1' | readVal_interrupt2=='2'){}
//	letterEntered=false;
//	count=0;

//fxnAcceptsSingleCharTypeAndReturnNothing(c);
//fxnAcceptsSingleCharTypeAndReturnNothing(i);


if(i=='0'){
	volatile char turnOffLED[]={'T','u','r','n',' ','O','F','F',':','\n'};
	GPIOC->ODR = (GPIOC->ODR & 0xFFFFFC3F);
	fxnAcceptsArrayOfCharsAndReturnNothing(turnOffLED);
	(c=='r' ? fxnAcceptsArrayOfCharsAndReturnNothing(redLED) : (c=='b' ? fxnAcceptsArrayOfCharsAndReturnNothing(blueLED) : (c=='o' ? fxnAcceptsArrayOfCharsAndReturnNothing(orangeLED) : (c=='g' ? fxnAcceptsArrayOfCharsAndReturnNothing(greenLED) : fxnAcceptsArrayOfCharsAndReturnNothing(errorMessageWrongButton))) ));
...
//	count=0;
	...
	volatile char turnedOnLED[]={'T','u','r','n',' ','O','N',':',' '};
	fxnAcceptsArrayOfCharsAndReturnNothing(turnedOnLED);
	(c=='r' ? (fxnAcceptsArrayOfCharsAndReturnNothing(redLED),GPIOC->ODR =(0x40 | (GPIOC->ODR & 0xFFFFFC3F) )) : (c=='b' ? (fxnAcceptsArrayOfCharsAndReturnNothing(blueLED),GPIOC->ODR =(0x80 | (GPIOC->ODR & 0xFFFFFC3F) )) : (c=='o' ? (fxnAcceptsArrayOfCharsAndReturnNothing(orangeLED),GPIOC->ODR =(0x100 | (GPIOC->ODR & 0xFFFFFC3F) )) : (c=='g' ? (fxnAcceptsArrayOfCharsAndReturnNothing(greenLED),GPIOC->ODR =(0x200 | (GPIOC->ODR & 0xFFFFFC3F) )) : (fxnAcceptsArrayOfCharsAndReturnNothing(errorMessageWrongButton))) )));
	
//	letterEntered=false;
//	count=0;
}
else if (i=='2'){ //if i=2:
	volatile char toggleedLED[]={'T','o','g','g','l','e',':',' '};
	fxnAcceptsArrayOfCharsAndReturnNothing(toggleedLED);
switch (c) {
case ('r'):
{fxnAcceptsArrayOfCharsAndReturnNothing(redLED);((GPIOC->ODR == (0x40 | (GPIOC->ODR & 0xFFFFFC3F) )) ? (GPIOC->ODR &= 0xFFFFFFBF) : (GPIOC->ODR =(0x40 | (GPIOC->ODR & 0xFFFFFC3F) )));} break;	//letterEntered=false;
case ('b'):
{fxnAcceptsArrayOfCharsAndReturnNothing(blueLED);((GPIOC->ODR == (0x80 | (GPIOC->ODR & 0xFFFFFC3F) )) ? (GPIOC->ODR &= 0xFFFFFF7F) : (GPIOC->ODR =(0x80 | (GPIOC->ODR & 0xFFFFFC3F) )));}	break;	 //letterEntered=false;
case ('o'):
{fxnAcceptsArrayOfCharsAndReturnNothing(orangeLED);((GPIOC->ODR == (0x100 | (GPIOC->ODR & 0xFFFFFC3F) )) ? (GPIOC->ODR &= 0xFFFFFEFF) : (GPIOC->ODR =(0x100 | (GPIOC->ODR & 0xFFFFFC3F) ))); }break;	//letterEntered=false;
case ('g'):
{fxnAcceptsArrayOfCharsAndReturnNothing(greenLED);((GPIOC->ODR == (0x200 | (GPIOC->ODR & 0xFFFFFC3F) )) ? (GPIOC->ODR &= 0xFFFFFDFF) : (GPIOC->ODR =(0x200 | (GPIOC->ODR & 0xFFFFFC3F) )));	}break; //letterEntered=false;
default:
{GPIOC->ODR = (GPIOC->ODR & 0xFFFFFC3F);	
	fxnAcceptsArrayOfCharsAndReturnNothing(errorMessageWrongButton); 
}
...
//	count=0;
	...
	letterEntered=false;
	count=0;
	fxnAcceptsArrayOfCharsAndReturnNothing(errorMustBe012);
}
		...
		// PC9 is connected to GREEN LED.
				...
				//Part 2: 3. Setup the blank interrupt handler. 
...
//char USART3_4_IRQHandler(void){


//if(count==0){
// //USART3->RQR |=0x8; // RXFRQ flush

//			readVal_interrupt1=USART3->RDR; 
//			fxnAcceptsSingleCharTypeAndReturnNothing(readVal_interrupt1);
//			 count++;
//	

//	//return readVal_interrupt1;
//		 }
//else if(count==1){

//	readVal_interrupt2=USART3->RDR; 
//	fxnAcceptsSingleCharTypeAndReturnNothing(readVal_interrupt2);
////	fxnAcceptsSingleCharTypeAndReturnNothing('\n');
//	count++;
//  letterEntered=true;
//	return readVal_interrupt2;
//	
//}
...
turn_On_Off_Or_toggleLEDsWithTheFirstColorLetterAnd0To2Digits(readVal_interrupt1,readVal_interrupt2);
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
	...
	//PART 2. : 1. In the USART initialization, enable the receive register not empty interrupt.
...
//			PART 2 : 2. Enable and set the USART interrupt priority in the NVIC.
//if(letterEntered==false){}
            NVIC_EnableIRQ(USART3_4_IRQn); //
            NVIC_SetPriority(USART3_4_IRQn,1); // set priority
		
						
char commandPromptCMD[]={'C','M','D','?','\n'};					
	
	
//char turnOffLED[]={'T','u','r','n',' ','O','F','F',':','\n'};
//char redLED[]={'R','e','d',' ','L','E','D','\n'};
//char blueLED[]={'B','l','u','e',' ','L','E','D','\n'};
//char orangeLED[]={'O','r','a','n','g','e',' ','L','E','D','\n'};
//char greenLED[]={'G','r','e','e','n',' ','L','E','D','\n'};
  /* Infinite loop */
...
//Part 2: 2. Print a command prompt such as “CMD?” when waiting for user input.
//if(count==-1){	count++;
//}		
//if(letterEntered==false){}


//while( letterEntered==false){ //wait until it's set.(Bit 5 RXNE: Read data register not empty)
fxnAcceptsArrayOfCharsAndReturnNothing(commandPromptCMD);
// }
...
//if(readVal_interrupt2 !='0' | readVal_interrupt2 !='1' | readVal_interrupt2 !='2' ){
//	letterEntered=false;
//	count=0;
//	fxnAcceptsArrayOfCharsAndReturnNothing(errorMustBe012);
//}
//else if(readVal_interrupt2=='0'){
//	GPIOC->ODR = (GPIOC->ODR & 0xFFFFFC3F);
//	fxnAcceptsArrayOfCharsAndReturnNothing(turnOffLED);
//	(readVal_interrupt1=='r' ? fxnAcceptsArrayOfCharsAndReturnNothing(redLED) : (readVal_interrupt1=='b' ? fxnAcceptsArrayOfCharsAndReturnNothing(blueLED) : (readVal_interrupt1=='o' ? fxnAcceptsArrayOfCharsAndReturnNothing(orangeLED) : (readVal_interrupt1=='g' ? fxnAcceptsArrayOfCharsAndReturnNothing(greenLED) : fxnAcceptsArrayOfCharsAndReturnNothing(errorMessageWrongButton))) ));
//}

//else if(readVal_interrupt2=='1'){
//	fxnAcceptsArrayOfCharsAndReturnNothing(turnedOnLED);
//	(readVal_interrupt1=='r' ? (fxnAcceptsArrayOfCharsAndReturnNothing(redLED),GPIOC->ODR =(0x40 | (GPIOC->ODR & 0xFFFFFC3F) )) : (readVal_interrupt1=='b' ? (fxnAcceptsArrayOfCharsAndReturnNothing(blueLED),GPIOC->ODR =(0x80 | (GPIOC->ODR & 0xFFFFFC3F) )) : (readVal_interrupt1=='o' ? (fxnAcceptsArrayOfCharsAndReturnNothing(orangeLED),GPIOC->ODR =(0x100 | (GPIOC->ODR & 0xFFFFFC3F) )) : (readVal_interrupt1=='g' ? (fxnAcceptsArrayOfCharsAndReturnNothing(greenLED),GPIOC->ODR =(0x200 | (GPIOC->ODR & 0xFFFFFC3F) )) : (fxnAcceptsArrayOfCharsAndReturnNothing(errorMessageWrongButton))) )));
//}
//else if (readVal_interrupt2=='2'){ //if i=2:
//	fxnAcceptsArrayOfCharsAndReturnNothing(toggleedLED);
//switch (readVal_interrupt2) {
//case ('r'):
//{fxnAcceptsArrayOfCharsAndReturnNothing(redLED);	((GPIOC->ODR == (0x40 | (GPIOC->ODR & 0xFFFFFC3F) )) ? (GPIOC->ODR &= 0xFFFFFFBF) : (GPIOC->ODR =(0x40 | (GPIOC->ODR & 0xFFFFFC3F) )));} break;	//letterEntered=false;
//case ('b'):
//{fxnAcceptsArrayOfCharsAndReturnNothing(blueLED);((GPIOC->ODR == (0x80 | (GPIOC->ODR & 0xFFFFFC3F) )) ? (GPIOC->ODR &= 0xFFFFFF7F) : (GPIOC->ODR =(0x80 | (GPIOC->ODR & 0xFFFFFC3F) )));}	break;	 //letterEntered=false;
...
//		 }
	...
	//	readChar();
//		 //---Activeate The Following Part to Test Part1 only --//
//		 
...
//		 
//		 //END: Activeate The Following Part to Test Part1 only// 
